...
class CheckDirectoryExistenceTestCase(TestCase):
    ...
    def test_directory_exists(self):
        self.assertTrue(
            check_directory_existence(self.test_vm_name_1, self.test_directory)
        )

    def test_directory_not_exists(self):
        self.assertFalse(
            check_directory_existence(self.test_vm_name_2, self.test_directory)
        )
...
class ApplyTerraformModuleTestCase(TestCase):
    ...
    def test_apply_terraform_module_success(
        self, mock_format_terraform_result, mock_manage_engine_note, mock_terraform
    ):
        ...
        mock_format_terraform_result.return_value = (
            {"add": 1, "change": 2, "destroy": 0},
            True,
        )

        result = apply_terraform_module(
            "mock_module_path", "mock_ticket_id", "mock_vm_name", created=True
        )

        mock_tf_instance.apply.assert_called_once_with(skip_plan=True)
        mock_manage_engine_note.assert_called_once_with(
            "mock_ticket_id", "VM mock_vm_name created successfully."
        )
    ...
    @patch("common.modules.manage_engine.ManageEngine.add_note_to_ticket")
    @patch("common.modules.terraform_utils.format_terraform_result")
    def test_apply_terraform_module_failure(
        self, mock_format_terraform_result, mock_manage_engine_note, mock_terraform
    ):
        mock_tf_instance = MagicMock()
        ...
        mock_tf_instance.apply.assert_called_once_with(skip_plan=True)
        mock_manage_engine_note.assert_called_once_with(
            "mock_ticket_id", "Create VM mock_vm_name failed."
        )
...
class GetModulePathTestCase(TestCase):
    def setUp(self):
        ...
        self.exist_vm_path = os.path.join(
            self.test_directory, self.test_vm_dir, self.test_vm_name
        )
...
class KeepOldVersionTestCase(TestCase):
    ...
    def test_keep_old_version_success(self, mock_copy, mock_makedirs):
        ...
        mock_makedirs.assert_called_once_with(
            f"{self.test_directory}/old", exist_ok=True
        )
